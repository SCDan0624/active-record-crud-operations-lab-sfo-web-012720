...
def can_be_created_in_a_block(args = { title: "Home Alone", release_date: 1990 }) #fixed
  # If no arguments are passed, use default values:
  # title == "Home Alone"
  # release_date == 1990
  
  Movie.create do |m| #fixed
    m.title = args[:title]
...
def can_find_by_multiple_attributes
  ...
  # release_date == 2000
  # director == "Me"
  Movie.find_by(title: "Title", release_date: 2000, director: "Me") #fixed
end

def can_find_using_where_clause_and_be_sorted
  # For this test return all movies released after 2002 and ordered by 
  # release date descending
  Movie.where("release_date > 2002").order(release_date: :desc) #fixed
end

def can_be_found_updated_and_saved #fixed
...
def can_update_using_update_method #fixed
...
def can_update_multiple_items_at_once #fixed
